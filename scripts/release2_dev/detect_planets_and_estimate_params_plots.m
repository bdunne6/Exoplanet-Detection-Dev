...
plot_dest = fullfile('plots','detection_plots1');


%% main script %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~exist(plot_dest,'dir')
    mkdir(plot_dest);
end
...
stemp = cat(2,cat(1,img_set.images).meta)';
%file_names = {cat(2,cat(1,img_set.images).meta).file_name};

file_names = arrayfun(@(x)(x.file_name),stemp,'UniformOutput',false);
i_match = contains(file_names,pat);
i_match = all(i_match,2);
...
for i0 = 1:numel(img_set.images)
    ...
    imgt_1 = res1_0 - medfilt2(res1_0,[17,17]);
    pdet1 = mf_planet_detector(imgt_1,psf1,[3,17],3);
    ...
    imgt_2 = res2_0 - medfilt2(res2_0,[17,17]);
    pdet2 = mf_planet_detector(imgt_2,psf2,[3,17],3);
    ...
    figure;
    set(gcf,'Position', [550 685 1323 671])
    msize = 13;
    t2 = tiledlayout(2,3);
    t2.Padding = 'compact';
    ...
    colorbar;
    hold on;
    cent1=  pdet1.detections.locations;
    plot(cent1(:,1),cent1(:,2),'.r','MarkerSize',msize);

    title('Foreground Image','Interpreter','none');

    nexttile(tile_2d(2,1))
    imagesc(res2_0)
    colorbar;
    hold on;
    cent2=  pdet2.detections.locations;
    plot(cent2(:,1),cent2(:,2),'.r','MarkerSize',msize);
    title('Foreground Image','Interpreter','none');

    nexttile(tile_2d(1,2))
    imagesc(pdet1.img_mf)
    hold on;
    cent1=  pdet1.detections.locations;
    plot(cent1(:,1),cent1(:,2),'.r','MarkerSize',msize);
    plot(pdet1.candidates.locations(:,1),pdet1.candidates.locations(:,2),'or');
    legend('detection','local maxima')
    colorbar;
    title('Matched Filter Output With Detections','Interpreter','none');

    nexttile(tile_2d(2,2))
    imagesc(pdet2.img_mf)
    hold on;
    cent2=  pdet2.detections.locations;
    plot(cent2(:,1),cent2(:,2),'.r','MarkerSize',msize);
    plot(pdet2.candidates.locations(:,1),pdet2.candidates.locations(:,2),'or');
    legend('detection','local maxima')
    colorbar;
    title('Matched Filter Output With Detections','Interpreter','none');

    nexttile(tile_2d(1,3))
    histogram(pdet1.candidates.intensities,15)
    axv = ylim();
    xv = pdet1.candidates.upper_thresh;
    hold on;
    plot([xv,xv],axv, '-r','Linewidth',2);
    ylim(axv);
    grid on;
    title('Histogram of Local Maxima Intensities','Interpreter','none');
    xlabel('Matched filter intensity (normalized counts)')
    ylabel('Count')
    legend('count','threshold')

    nexttile(tile_2d(2,3))
    histogram(pdet2.candidates.intensities,15)
    axv = ylim();
    xv = pdet2.candidates.upper_thresh;
    hold on;
    plot([xv,xv],axv, '-r','Linewidth',2);
    ylim(axv);
    ...
    plot_name = extractBefore(img_set.images(i0).meta(1).file_name,'.fits');

    plot_path = fullfile(plot_dest,[plot_name,'.png']);
