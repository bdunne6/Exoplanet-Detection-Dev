...
%% user settings %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
path_struct(1).instrument_scenario = '1e10';
path_struct(1).mdl_path = fullfile('ica_models','ica_17_ec2012725779cfc8a01eb63c55e867a7.mat');
path_struct(1).input_data_path = fullfile(mat_output_root,'img_set_disk_1em10_rev4.mat');
path_struct(1).label_path = 'planet_labels_1e10.mat';
path_struct(1).output_data_path = fullfile(mat_output_root,'img_set_disk_1em10_rev5.mat');

path_struct(2).instrument_scenario = '1e9';
path_struct(2).mdl_path = fullfile('ica_models_1em9','ica_19_b7d23f1924ea919bc6a3620b473a79bc.mat');
path_struct(2).input_data_path = fullfile(mat_output_root,'img_set_disk_1em9_rev4.mat');
path_struct(2).label_path = 'planet_labels_1em9.mat';
path_struct(2).output_data_path = fullfile(mat_output_root,'img_set_disk_1em9_rev5.mat');
...
for i0 = 2:numel(path_struct)
    ...
    load(path_struct(i0).input_data_path);
    %img_set = img_set.stack_by({'passband'});
    ...
    load(path_struct(i0).label_path);
    ...
    for i1 = 1:numel(img_set.images)
        ...
        labels_1 = find(ismember({planet_labels.file_name},img_set.images(i1).meta(1).file_name));
        labels_1 = planet_labels(labels_1);
        labels_1 = labels_1([labels_1.button] <= 2);

        labels_2 = find(ismember({planet_labels.file_name},img_set.images(i1).meta(2).file_name));
        labels_2 = planet_labels(labels_2);
        labels_2 = labels_2([labels_2.button] <= 2);

        img_sample = img_set.images(i1).data_roi;
        ...
        colorbar;
        title_str = img_set.images(i1).meta(1).file_name(8:end-5);
        ...
        colorbar;
        title_str = img_set.images(i1).meta(2).file_name(8:end-5);
        title(title_str,'Interpreter','none');
        ...
        raw_pix1 = img_set.images(i1).data(:,:,1);
        ...
        n0 = numel(labels_1);
        for i3 = 1:size(cent_xy,1)
            labels_1(n0+i3).x = cent_xy(i3,1);
            labels_1(n0+i3).y = cent_xy(i3,2);
        end
        ...
        if ~isempty(labels_1)
            ...
            for i3 = 1:numel(labels_1)
                cent0 = [labels_1(i3).x,labels_1(i3).y];
                w=7;
                [cent1,cent_fit1] = refine_centroid_gaussian(res1_0 ,cent0,w,sigma_lookup1);
                cent_uncertainty = diff(cent_fit1.ci(2:3,:),1,2)/2;
                labels_1(i3).x_r = cent1(1);
                labels_1(i3).y_r = cent1(2);
                labels_1(i3).x_u = cent_uncertainty(1);
                labels_1(i3).y_u = cent_uncertainty(2);
                labels_1(i3).counts = cent_fit1.counts;
                [~,~,noise_mag,noise_per_pixel] = estimate_counts(res1_0,cent1,5);
                labels_1(i3).counts_snr = labels_1(i3).counts/noise_mag;
        ...
        title(title_str);
        ax1.UserData = img_set.images(i1).meta(1).file_name;

        raw_pix2 = img_set.images(i1).data(:,:,2);
        ...
        n0 = numel(labels_2);
        for i3 = 1:size(cent_xy,1)
            labels_2(n0+i3).x = cent_xy(i3,1);
            labels_2(n0+i3).y = cent_xy(i3,2);
        end
        ...
        if ~isempty(labels_2)
            ...
            for i3 = 1:numel(labels_2)
                cent0 = [labels_2(i3).x,labels_2(i3).y];
                w=7;
                ...
                labels_2(i3).x_r = cent1(1);
                labels_2(i3).y_r = cent1(2);
                labels_2(i3).x_u = cent_uncertainty(1);
                labels_2(i3).y_u = cent_uncertainty(2);
                labels_2(i3).counts = cent_fit2.counts;
                [~,~,noise_mag,noise_per_pixel] = estimate_counts(res2_0,cent1,5);
                labels_2(i3).counts_snr = labels_2(i3).counts/noise_mag;
        ...
        title(title_str);
        drawnow();
        ax2.UserData = img_set.images(i1).meta(2).file_name;
        %pause(0.1)

        img_set.images(i1).meta(1).mf_detections = pdet1;
        img_set.images(i1).meta(2).mf_detections = pdet2;

        img_set.images(i1).meta(1).background_estimate = reci1_2(:,:,1);
        img_set.images(i1).meta(2).background_estimate = reci1_2(:,:,2);

        img_set.images(i1).meta(1).planet_locations = rmfield(labels_1,'file_name');
        img_set.images(i1).meta(2).planet_locations = rmfield(labels_2,'file_name');

        img_set.images(i1).meta(1).disk.magnitude_counts = disk_mag1;
        img_set.images(i1).meta(2).disk.magnitude_counts = disk_mag2;
    end
    img_set = add_final_params(img_set);
    save(path_struct(i0).output_data_path,'img_set');
end
